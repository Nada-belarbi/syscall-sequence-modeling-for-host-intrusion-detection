...
def main() -> None:
    ...
    for key, path in protocol_paths.items():
        ...
        before_sha.to_csv(metadata_root / f"adfa_leakage_sha1_checks_protocol_{key}_before_fix.csv", index=False)
        before_tr.to_csv(metadata_root / f"adfa_leakage_traceid_checks_protocol_{key}_before_fix.csv", index=False)
        after_sha.to_csv(metadata_root / f"adfa_leakage_sha1_checks_protocol_{key}_after_fix.csv", index=False)
        after_tr.to_csv(metadata_root / f"adfa_leakage_traceid_checks_protocol_{key}_after_fix.csv", index=False)
        indices[key] = df_fixed

    # Keep compatibility names used by notebooks.
    split_overlap_counts(indices["a"], key_col="sha1").to_csv(metadata_root / "adfa_leakage_sha1_checks.csv", index=False)
    split_overlap_counts(indices["a"], key_col="trace_id").to_csv(metadata_root / "adfa_leakage_traceid_checks.csv", index=False)

    ratio_tbl = indices["a"].groupby(["split", "label"]).size().unstack(fill_value=0)
    ratio_tbl["attack_to_normal_ratio"] = ratio_tbl["attack"] / ratio_tbl["normal"].clip(lower=1)
    ratio_tbl["attack_pct"] = (ratio_tbl["attack"] / (ratio_tbl["attack"] + ratio_tbl["normal"]).clip(lower=1)) * 100.0
    ratio_tbl.reset_index().to_csv(metadata_root / "adfa_split_ratio_protocol_a.csv", index=False)
    ...
    scenario_split_tbl.to_csv(metadata_root / "adfa_attack_scenario_split_protocol_a.csv", index=False)
    ...
    from src.data_loading import parse_syscall_file  # local import to avoid side effects at module import time

    valid_df = valid_df.copy()
    valid_df["raw_sequence"] = [sanitize_sequence(parse_syscall_file(Path(p), strict=True)) for p in valid_df["filepath"]]
    ...
    if frozen_cfg_path.exists():
        ...
        max_len = int(frozen.get("max_len_choice_summary", {}).get("selected_max_len", max_len))
    ...
    valid_df["encoded_post"] = valid_df["encoded"].map(
        lambda s: pad_or_truncate(s, max_len=max_len, pad_value=pad_value, truncation="post", padding="post")
    )
    valid_df["encoded_pre"] = valid_df["encoded"].map(
        lambda s: pad_or_truncate(s, max_len=max_len, pad_value=pad_value, truncation="pre", padding="post")
    )
    ...
    frozen = {
        ...
        "sha1_overlap_after_fix": split_overlap_counts(indices["a"], key_col="sha1").to_dict(orient="records"),
        "traceid_overlap_after_fix": split_overlap_counts(indices["a"], key_col="trace_id").to_dict(orient="records"),
    }
    (metadata_root / "frozen_preprocessing_config.json").write_text(json.dumps(frozen, indent=2, ensure_ascii=True), encoding="utf-8")

    # Keep compatibility root files updated to fixed version too.
    index_to_save.to_parquet(processed_root / "adfa_processed_index.parquet", index=False)
    np.save(processed_root / "adfa_sequences_post.npy", np.array(valid_df["encoded_post"].tolist(), dtype=np.int32))
    np.save(processed_root / "adfa_sequences_pre.npy", np.array(valid_df["encoded_pre"].tolist(), dtype=np.int32))
