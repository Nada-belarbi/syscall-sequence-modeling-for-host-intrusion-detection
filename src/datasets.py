...
class SyscallSequenceDataset(Dataset):
    ...
    def __init__(
        ...
        self.sample_ids = (
            ...
            else [str(i) for i in range(len(labels))]
        )
        self.metadata = (
            metadata if metadata is not None else [{} for _ in range(len(labels))]
        )
...
def make_dataloader(
    ...
    return DataLoader(
        ...
        collate_fn=lambda b: collate_padded_batch(
            b, pad_value=pad_value, sort_by_length=sort_by_length
        ),
    )
