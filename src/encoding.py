...
def make_bow_matrix(
    ...
    matrix = csr_matrix(
        ...
        shape=(len(seq_list), len(vocabulary)),
    )
...
def build_baseline_feature_sets(
    ...
    bg_train, bg_vocab = make_ngram_count_matrix(
        train_sequences, ngram_range=(2, 2), vocabulary=None
    )
    bg_val, _ = make_ngram_count_matrix(
        val_sequences, ngram_range=(2, 2), vocabulary=bg_vocab
    )
    bg_test, _ = make_ngram_count_matrix(
        test_sequences, ngram_range=(2, 2), vocabulary=bg_vocab
    )
