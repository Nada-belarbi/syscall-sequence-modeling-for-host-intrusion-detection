...
def discover_adfa_samples(
	...
	def add_row(file_path: Path, source_split: str, attack_family: str | None, attack_scenario: int | None) -> None:
...
def assign_protocol_main(samples: pd.DataFrame, validation_normal_ratio: float = 0.5) -> pd.DataFrame:
	...
	hash_values = df.loc[is_normal_validation, "rel_path"].map(_stable_hash_to_unit_interval)
...
def assign_protocol_baseline_binary(
	...
	sss_train = StratifiedShuffleSplit(n_splits=1, train_size=train_size, random_state=random_state)
	train_idx, temp_idx = next(sss_train.split(idx.reshape(-1, 1), labels))

	temp_labels = labels[temp_idx]
	val_ratio_within_temp = val_size / (val_size + test_size)
	sss_val = StratifiedShuffleSplit(n_splits=1, train_size=val_ratio_within_temp, random_state=random_state)
...
def assign_protocol_heldout_families(samples: pd.DataFrame, heldout_families: list[str]) -> pd.DataFrame:
...
def compute_sequence_lengths(samples: pd.DataFrame, strict: bool = True) -> pd.DataFrame:
	...
	for row in df.itertuples(index=False):
		...
		try:
			...
			records.append({"trace_id": row.trace_id, "length": len(seq), "is_empty": len(seq) == 0, "has_parse_error": False, "parse_error": ""})
		except Exception as exc:  # noqa: BLE001
			records.append({"trace_id": row.trace_id, "length": -1, "is_empty": False, "has_parse_error": True, "parse_error": str(exc)})
	metrics = pd.DataFrame(records)
	base_cols = [c for c in df.columns if c not in {"length", "is_empty", "has_parse_error", "parse_error"}]
...
def leakage_report(index_df: pd.DataFrame) -> dict[str, object]:
	...
	if "attack_family" in index_df.columns:
		family_split = index_df.groupby(["split", "attack_family"]).size().reset_index(name="count") if "split" in index_df.columns else pd.DataFrame()
		report["family_split_table"] = family_split.to_dict(orient="records") if not family_split.empty else []
...
def split_overlap_counts(index_df: pd.DataFrame, key_col: str = "sha1") -> pd.DataFrame:
	...
	for i, a in enumerate(splits):
		for b in splits[i + 1 :]:
			set_a = set(index_df.loc[index_df["split"].eq(a), key_col].dropna().tolist())
			set_b = set(index_df.loc[index_df["split"].eq(b), key_col].dropna().tolist())
			rows.append({"pair": f"{a}-{b}", f"{key_col}_overlap_count": len(set_a.intersection(set_b))})
...
def enforce_sha1_disjoint_splits(
	...
	for sha1, group in df.groupby("sha1", sort=False):
		group_splits = [s for s in group["split"].dropna().astype(str).tolist() if s in priority_rank]
		if not group_splits:
			continue
...
def dataset_audit_report(index_df: pd.DataFrame) -> dict[str, object]:
	...
	result["num_by_source_split"] = index_df["source_split"].value_counts(dropna=False).to_dict()
	if "split" in index_df.columns:
		result["num_by_split"] = index_df["split"].value_counts(dropna=False).to_dict()
	if "attack_family" in index_df.columns:
		fam = index_df[index_df["label"] == "attack"]["attack_family"].value_counts(dropna=False)
	...
	result["num_empty_sequences"] = int(index_df["is_empty"].sum()) if "is_empty" in index_df.columns else 0
	result["num_parse_errors"] = int(index_df["has_parse_error"].sum()) if "has_parse_error" in index_df.columns else 0
	result["leakage_checks"] = leakage_report(index_df)
	return result


def assign_research_split(samples: pd.DataFrame, validation_normal_ratio: float = 0.5) -> pd.DataFrame:
	"""Backward-compatible alias for the principal protocol."""
	return assign_protocol_main(samples=samples, validation_normal_ratio=validation_normal_ratio)
