...
def choose_max_len(
    lengths: Iterable[int], quantile: float = 0.95, min_cap: int = 32
) -> int:
...
def suggest_truncation_lengths(
    lengths: Iterable[int], candidates: list[float] | None = None
) -> dict[str, int]:
...
def truncate_sequence(
    sequence: list[int], max_len: int, strategy: str = "post"
) -> list[int]:
...
def pad_sequence(
    sequence: list[int], max_len: int, pad_value: int = 0, strategy: str = "post"
) -> list[int]:
